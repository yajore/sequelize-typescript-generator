...
export class ModelBuilder extends Builder {
  ...
  private static buildColumnPropertyDecl(
    ...
    const buildColumnDecoratorProps = (
      ...
      const props: Partial<ModelAttributeColumnOptions> = {
        ...
        ...(col.defaultValue !== undefined && {
          defaultValue: dialect.mapDefaultValueToSequelize(col.defaultValue),
        }),
      };
    ...
    const buildIndexDecoratorProps = (
      index: IIndexMetadata
    ): Partial<IndexOptions & IndexFieldOptions> => {
      const props: Partial<IndexOptions & IndexFieldOptions> = {
        name: index.name,
        ...(index.using && { using: index.using }),
        ...(index.collation && {
          order: index.collation === "A" ? "ASC" : "DESC",
        }),
        unique: index.unique,
      };
    ...
    return ts.factory.createPropertyDeclaration(
      [
        ...(col.foreignKey
          ? [
              generateArrowDecorator(foreignKeyDecorator, [
                col.foreignKey.targetModel,
              ]),
            ]
          : []),
        generateObjectLiteralDecorator(
          "Column",
          buildColumnDecoratorProps(col)
        ),
        ...(col.indices && col.indices.length
          ? col.indices.map((index) =>
              generateObjectLiteralDecorator(
                "Index",
                buildIndexDecoratorProps(index)
              )
            )
          : []),
      ],
      ...
      ts.factory.createTypeReferenceNode(
        dialect.mapDbTypeToJs(col.type) ?? "any",
        undefined
      ),
      undefined
    );
  ...
  private static buildAssociationPropertyDecl(
    ...
    return ts.factory.createPropertyDeclaration(
      ...
      associationName.includes("Many")
        ? pluralize.plural(targetModel)
        : pluralize.singular(targetModel),
      ts.factory.createToken(ts.SyntaxKind.QuestionToken),
      associationName.includes("Many")
  ...
  private static buildTableClassDeclaration(
    ...
    [...importModels].forEach((modelName) => {
      generatedCode += nodeToString(
        generateNamedImports([modelName], `./${modelName}`)
      );
          ...
          ...Object.values(columns).map((c) =>
            ts.factory.createPropertySignature(
              ...
              c.autoIncrement || c.allowNull || c.defaultValue !== undefined
                ...
                : undefined,
              ts.factory.createTypeReferenceNode(
                dialect.mapDbTypeToJs(c.type) ?? "any",
                undefined
              )
            )
          ),
    ...
    const tableAlisProps: any = {};
    if (tableMetadata.paranoid) {
      tableAlisProps.paranoid = tableMetadata.paranoid;
    }
    if (tableMetadata.deletedAt) {
      tableAlisProps.deletedAt = tableMetadata.deletedAt;
    }
    if (tableMetadata.createdAt) {
      tableAlisProps.createdAt = tableMetadata.createdAt;
    }
    if (tableMetadata.updatedAt) {
      tableAlisProps.updatedAt = tableMetadata.updatedAt;
    }
    const classDecl = ts.factory.createClassDeclaration(
      ...
      [
        ...Object.values(columns).map((col) =>
          this.buildColumnPropertyDecl(col, dialect)
        ),
        ...(tableMetadata.associations && tableMetadata.associations.length
          ? tableMetadata.associations.map((a) =>
              this.buildAssociationPropertyDecl(a)
            )
          : []),
      ]
    );
  ...
  async build(): Promise<void> {
    ...
    if (Object.keys(tablesMetadata).length === 0) {
      console.warn(
        `Couldn't find any table for database ${this.config.connection.database} and provided filters`
      );
      return;
    }
    ...
    for (const tableMetadata of Object.values(tablesMetadata)) {
      ...
      const tableClassDecl = ModelBuilder.buildTableClassDeclaration(
        tableMetadata,
        this.dialect,
        this.config.strict
      );
      ...
      writePromises.push(
        (async () => {
          ...
          await fs.writeFile(outPath, tableClassDecl, { flag: "w" });
